#!/usr/bin/env bash
# mcp-server spike (task #41, ADR 0001 — wake = projection compiler): a runnable MCP
# server host that serves an wake emit-mcp tool catalog over a Fram CLAIM store.
#
...
#   2. generates a real emit-mcp catalog from demo/crm-v2.wake (falls back to a checked-in
#      catalog if wake-compile is unavailable in this env),
...
if [[ ! -s "$cat_json" ]]; then
  "$wake_web/bin/wake-compile" --mcp "$wake_web/demo/crm-v2.wake" > "$cat_json" 2>/dev/null \
    || echo "wake-compile --mcp unavailable; gate will use its hand-supplied crm-v2 catalog" >&2
fi
