...
// SEAM ASSERTIONS — both custom panes carved by eddy, fed by window.eddy.
const panesMounted = await page.evaluate(() => ({
  ...
  busHasStore: !!(window.eddy && window.eddy.stores && window.eddy.stores.agents),
}));

await page.click('#app .cursor-pointer');
await page.waitForTimeout(500);
const selectedUuid = await page.evaluate(() => window.eddy.selection.get());
