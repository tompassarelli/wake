...
const archiveName = "tompassarelli-wake-runtime-0.2.1.tgz";
const receiptName = "tompassarelli-wake-runtime-0.2.1.receipt.json";
...
function releaseSource(scratch) {
  ...
  run(["git", "commit", "-q", "-m", "Wake 0.2.1 runtime"], { cwd: seed, env: environment });
  ...
  const releaseTagObject = new TextDecoder().decode(run([
    "git", "rev-parse", "refs/tags/v0.2.1",
  ], { cwd: seed })).trim();
...
describe("@tompassarelli/wake-runtime package", () => {
  ...
  test("refuses untagged, lightweight-tagged, dirty, wrong-origin, and mismatched releases", () => {
    ...
    try {
      const source = releaseSource(scratch);
      run(["git", "tag", "-d", "v0.2.1"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.2.1",
        "--output", `${scratch}/untagged`,
      ]).exitCode).not.toBe(0);

      run(["git", "tag", "v0.2.1"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.2.1",
