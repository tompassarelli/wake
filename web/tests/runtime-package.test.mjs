...
const archiveName = "tompassarelli-wake-runtime-0.2.2.tgz";
const receiptName = "tompassarelli-wake-runtime-0.2.2.receipt.json";
...
const expectedFiles = [
  ...
  "package/store-gateway.mjs",
  "package/store-http.mjs",
...
function releaseSource(scratch) {
  ...
  run(["git", "commit", "-q", "-m", "Wake 0.2.2 runtime"], { cwd: seed, env: environment });
  run(["git", "tag", "-a", "v0.2.2", "-m", "Wake v0.2.2"], {
    cwd: seed,
  ...
  const releaseTagObject = new TextDecoder().decode(run([
    "git", "rev-parse", "refs/tags/v0.2.2",
  ], { cwd: seed })).trim();
...
describe("@tompassarelli/wake-runtime package", () => {
  ...
  test("refuses untagged, lightweight-tagged, dirty, wrong-origin, and mismatched releases", () => {
    ...
    try {
      const source = releaseSource(scratch);
      run(["git", "tag", "-d", "v0.2.2"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.2.2",
        "--output", `${scratch}/untagged`,
      ]).exitCode).not.toBe(0);

      run(["git", "tag", "v0.2.2"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.2.2",
      ...
      run(["git", "tag", "-a", "v0.2.3", "-m", "Wake v0.2.3"], {
        cwd: source.first,
        env: releaseTagEnvironment,
      });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.2.3",
