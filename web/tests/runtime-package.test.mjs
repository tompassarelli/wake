...
const archiveName = "tompassarelli-wake-runtime-0.1.3.tgz";
const receiptName = "tompassarelli-wake-runtime-0.1.3.receipt.json";
...
function releaseSource(scratch) {
  ...
  run(["git", "commit", "-q", "-m", "Wake 0.1.3 runtime"], { cwd: seed, env: environment });
  run(["git", "tag", "-a", "v0.1.3", "-m", "Wake v0.1.3"], {
    cwd: seed,
  ...
  const releaseTagObject = new TextDecoder().decode(run([
    "git", "rev-parse", "refs/tags/v0.1.3",
  ], { cwd: seed })).trim();
...
describe("@tompassarelli/wake-runtime package", () => {
  ...
  test("refuses untagged, lightweight-tagged, dirty, wrong-origin, and mismatched releases", () => {
    ...
    try {
      const source = releaseSource(scratch);
      run(["git", "tag", "-d", "v0.1.3"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.1.3",
        "--output", `${scratch}/untagged`,
      ]).exitCode).not.toBe(0);

      run(["git", "tag", "v0.1.3"], { cwd: source.first });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.1.3",
      ...
      run(["git", "tag", "-a", "v0.1.4", "-m", "Wake v0.1.4"], {
        cwd: source.first,
        env: releaseTagEnvironment,
      });
      expect(fails([
        packer, "--source-root", source.first, "--version", "v0.1.4",
